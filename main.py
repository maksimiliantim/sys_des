...
def create_budget_item_command(user_id, description, amount):
    ...
    producer.send('budget-items-topic', value=command)
    producer.flush()
    return item_id
...
def process_kafka_messages():
    ...
    for message in consumer:
        ...
        if command['type'] == 'CREATE_BUDGET_ITEM':
            data = command['data']
            new_item = BudgetItem(
                id=data['id'],
...
def create_budget_item():
    ...
    item_id = create_budget_item_command(user.id, description, amount)
