workspace {
    ...
    model {
        ...
        budgeting = softwareSystem "Budgeting System" {
            ...
            budgetService = container "Budget Service" {
                description "Сервис управления бюджетными записями (CQRS)"
                technology "Java Spring Boot"
            }
            ...
            kafkaService = container "Kafka Service" {
                description "Сервис, обрабатывающий сообщения из Kafka и записывающий данные в базы данных"
            ...
            // Kafka messaging interactions
            budgetService -> kafkaService "Отправка сообщений о создании новых бюджетных записей" "Kafka"
            kafkaService -> budgetDB "Обработка сообщений и запись данных" "JDBC"
            ...
            budgetService -> kafkaService "Отправка сообщения о создании бюджетной записи"
            kafkaService -> budgetDB "INSERT INTO budget_items"
    ...
    views {
        ...
        dynamic budgeting "createBudgetItem" "Создание бюджетной записи" {
            ...
            budgeting.budgetService -> budgeting.kafkaService "Отправка сообщения о создании бюджетной записи"
            budgeting.kafkaService -> budgeting.budgetDB "INSERT INTO budget_items"
            autolayout lr
        }
